...
# for arduino and LED control with PWM
import serial
import time
...
sys.path.append('C:/Users/z5345591/PycharmProjects/PyNE-wells/code/DLL64')  # Add the path of the library here
sys.path.append('C:/Users/z5345591/PycharmProjects/PyNE-wells/code/Python_64')  # Add the path of the LoadElveflow.py
Calib_path = (
    'C:\\Users\\z5345591\\PycharmProjects\\PyNE-wells\\code\\Calib1.txt')  # Load calibration path/ new calibration path
camera_path = r"C:/Users/z5345591/PycharmProjects/PyNE-wells/code/x64/ASICamera2.dll"  # ZWO ASI camera DLL path
folder = r'C:/Users/z5345591/PycharmProjects/PyNE-wells/code/saved_images/'  # Your captured images will be saved here
main_folder = r"C:/Users/z5345591/PycharmProjects/PyNE-wells/code"  # main folder
KDCfolder = r"C:/Users/z5345591/PycharmProjects/PyNE-wells/code/Kinesis"
Autofocus_temp_images = r"C:/Users/z5345591/PycharmProjects/PyNE-wells/code/Autofocus_temp"
...
mux_com_port = str(
    "ASRL7::INSTR")  # COM port to which MUX Distributor is connected to. Refer to Elveflow SDK userguide for this!
...
Arduino_port = str("COM4")  # COM port to which Arduino is connected to
# Set up the serial connection to the Arduino
arduino = serial.Serial(Arduino_port, 9600)  # Replace 'COM3' with your correct port
...
buttonCommit34 = Label(root, height=5, width=30,
                       text="NB: To create a statioinay chamber \n with NO FLUID FLOW, please set the \n flow rate to 'Zero' ",
                       font='Helvetica 9 bold')
buttonCommit34.place(x=1430, y=680)

####### Run sequence
buttonCommit35 = Label(root, height=5, width=20, text="Step 3.1: Run GEN4 MUX ", font='Helvetica 12 bold')
buttonCommit35.place(x=550, y=720)

####### LED1B Label
buttonCommit36 = Label(root, height=3, width=23, text="THORLABS LED1B \n Control ", font='Helvetica 18 bold')
buttonCommit36.place(x=1500, y=100)
...
def MUX_home_valve():
    Answer = (c_char * 40)()
    error = MUX_DRI_Send_Command(Instr_ID.value, 0, Answer,
                                 40)  # length is set to 40 to contain the whole Serial Number
...
def MUX_set_valve():
    ...
    MUX_DRI_Set_Valve(Instr_ID.value, Valve2,
                      0)  # you can select valve rotation way, either shortest, clockwise or counter clockwise (only for MUX Distribution and Recirculation)
...
# %matplotlib
# get_ipython().run_line_magic('matplotlib', 'inline') # Use this instead https://stackoverflow.com/questions/44225002/why-doesnt-matplotlib-inline-work-in-python-script
import random
...
def animate(i):
    ...
    data_sens = c_double()
    error = OB1_Get_Press(Instr_ID.value, set_channel, 1, byref(Calib), byref(get_pressure),
                          1000)  # Acquire_data=1 -> read all the analog values
    error1 = OB1_Get_Sens_Data(Instr_ID.value, set_channel, 1,
...
def save_image_8bit():
    ...
    camera = asi.Camera(0)
    camera.set_roi(start_x=None, start_y=None, width=int(textBox14.get("1.0", "end-1c")),
                   height=int(textBox15.get("1.0", "end-1c")), bins=None, image_type=None)
...
def save_image_8bit_as():
    ...
    root.after(int(textBox11.get("1.0", "end-1c")) * 3600 * 1000, root.destroy)


my_btn16 = Button(root, text="Sequential Image \n Acquisition (8 bit)",
                  command=lambda: [save_image_8bit_as(), start_thread2()]).place(x=1450, y=810)
...
my_btn17 = Button(root, text="Sequential Image \n Acquisition (16 bit)", command=save_image_16bit_as).place(x=1445,
                                                                                                            y=860)
...
def control_led(led_number, state):
    """
    Control one of the two LEDs via Arduino.
    led_number: 1 or 2 (which LED to control)
    state: 'on' or 'off' (desired state of the LED)
    """
    if led_number == 1 and state == 'on':
        arduino.write(b'1')  # Turn on LED 1
    elif led_number == 1 and state == 'off':
        arduino.write(b'2')  # Turn off LED 1
    elif led_number == 2 and state == 'on':
        arduino.write(b'3')  # Turn on LED 2
    elif led_number == 2 and state == 'off':
        arduino.write(b'4')  # Turn off LED 2
    else:
        print("Invalid LED number or state.")


def LED1_on():
    control_led(1, 'on')  # Turn on LED 1


my_btn29 = Button(root, text="LED1 (490nm)\n ON", command=LED1_on).place(x=1600, y=200)


def LED1_off():
    control_led(1, 'off')  # Turn on LED 1


my_btn30 = Button(root, text="LED1 (490nm)\n OFF", command=LED1_off).place(x=1700, y=200)


def LED2_on():
    control_led(2, 'on')  # Turn on LED 1


my_btn31 = Button(root, text="LED2 (660nm)\n ON", command=LED2_on).place(x=1600, y=250)
...
my_btn32 = Button(root, text="LED2 (6600nm)\n OFF", command=LED2_off).place(x=1700, y=250)


# def Arduino_on():
#     arduino = serial.Serial(Arduino_port, 9600) # Replace 'COM3' with your correct port
#     time.sleep(2) # Allow some time for the connection to initialize
# my_btn33= Button(root, text="Open Serial", command=Arduino_on).place(x=1600, y= 300)

# def Arduino_off():
#     arduino.open()
# my_btn34= Button(root, text="Close Serial", command=Arduino_off).place(x=1700, y= 300)
...
#     # Move to slider slot 1 (illuminatin: 490nm, filter =535nm)
#     ELL9K_slot4()
#     time.sleep(5)
#     control_led(1, 'on')  # Turn on LED 1 (490nm)
#
...
#     filename_img = 'image_mono.tiff'
#     camera.capture(filename=filename_img)
#     os.rename(folder + filename_img, folder + "488_" + str(date_string) + ".tiff")
#     os.chdir(main_folder)
#
#     control_led(1, 'off')  # Turn off LED 1
#
#
...
#     root.after(int(textBox11.get("1.0", "end-1c")) * 60000, root.destroy)  # 3600*1000
#
#
# my_btn26 = Button(root, text="Sequential Image \n Acquisition (16 bit) \n Alexa 488 + 647",command=lambda: [save_image_16bit_slot_stage(), start_thread2()]).place(x=1445, y=910)
...
def KDC101():
    ...
    if sys.version_info < (3, 8):
        ...
        os.add_dll_directory(KDCfolder)

    lib: CDLL = cdll.LoadLibrary("Thorlabs.MotionControl.KCube.DCServo.dll")

    # Set constants
    serial_num = c_char_p(KDC101_serial_num)  # Add your device Serial number5
...
def save_image_16bit_slot_stage():
    ...
    # #Employ shutter to stop excessive photobleaching
    ELL9K_slot4()
    control_led(1, 'off')  # Turn on LED 1 (490nm)



    # #Employ shutter to do nothing
    ELL9K_slot4()
    control_led(1, 'off')  # Turn on LED 1 (490nm)
    ...
    # If the autofocusing  & imaging time exceeds the user defined time interval    user_interval = int(textBox10.get("1.0", "end-1c"))
...
my_btn26 = Button(root, text="Sequential Image \n Acquisition (16 bit) \n Alexa 488 + 647",
                  command=lambda: [save_image_16bit_slot_stage(), start_thread2()]).place(x=1445, y=910)
...
#     valve2 = c_double()
#     Valve2 = int(1)
#     Valve2 = c_int32(Valve2)
#     MUX_DRI_Set_Valve(Instr_ID.value, Valve2,0)
#     i = 1
#     while i < 10:
#         print(i)
#         i += 1
...
# my_btn28= Button(root, text="Sequence1", command=seq_flow).place(x=630, y= 780)
...
start2 = Button(root, text="Start Resistance Measurement", command=start_thread2).place(x=600, y=800)
# stop2 = Button(root, text="Stop Measurement", command=stop2).place(x=600, y=830)
